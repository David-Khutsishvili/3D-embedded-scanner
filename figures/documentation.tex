...
The scanner was controlled by an \textbf{Arduino Uno R4 WiFi}. Using its
standard digital I/O, PWM, and I2C peripherals. It was responsible for:
\begin{itemize}
...
with the known sensor geometry) helped measure the object's shape.
...
A measurement was only reported (and printed as a JSON line) when
\emph{both} sensors' average readings came back closer than the detection
threshold, \texttt{DETECTED\_THRESH\_MM}; if either sensor's reading was
farther than that, the point was dropped. This filtered out background
readings when the sensors were looking past the object at the empty
scanner frame. If an entire ring produced no detections at all, the scan
was considered complete and stopped early rather than continuing to sweep
empty air.
...
\begin{itemize}
  \item Adding more sensors around the plate for faster, more reliable
    scans -- more simultaneous viewpoints would reduce the number of
    rotation steps needed per ring and make the detection threshold less
    sensitive to a single sensor's noise.
  \item Post-processing the point cloud into a surface mesh instead of only visualizing raw points.
\end{itemize}
